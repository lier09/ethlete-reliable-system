  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 text-slate-900">
      ...
      <header className="bg-slate-900 text-white border-b border-slate-800 sticky top-0 z-40 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-lg bg-blue-600 flex items-center justify-center font-black text-xs text-white shadow-md shadow-blue-500/30">
                ⚡
              </div>
              <div>
            ...
            {/* Navigation Tabs */}
            <nav className="hidden md:flex items-center gap-1 bg-slate-800/90 p-1 rounded-xl border border-slate-700/60 text-xs">
              <button
            ...
            <div className="flex items-center gap-2.5">
              <button
                onClick={onOpenHelp}
                className="hidden sm:flex items-center gap-1 px-2.5 py-1.5 text-xs text-slate-300 hover:text-white hover:bg-slate-800 rounded-lg transition border border-slate-700"
                title="查看使用手册与判定原理"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-linear-to-r from-slate-800 to-slate-700 hover:from-slate-750 hover:to-slate-650 text-white border border-slate-600 rounded-xl text-xs font-bold transition shadow-sm group"
              >
                <Sliders className="w-3.5 h-3.5 text-blue-400 group-hover:rotate-45 transition-transform" />
                <span>进入科研控制中台</span>
        ...
        <div className="md:hidden flex items-center justify-around border-t border-slate-800 bg-slate-900/90 py-2 px-3 text-xs">
          <button
            onClick={() => setActiveClientTab('quick_eval')}
            className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-semibold ${
              activeClientTab === 'quick_eval' ? 'bg-blue-600 text-white' : 'text-slate-400'
            }`}
          >
          ...
          <button
            onClick={() => setActiveClientTab('team_roster')}
            className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-semibold ${
              activeClientTab === 'team_roster' ? 'bg-blue-600 text-white' : 'text-slate-400'
            }`}
          >
          ...
          <button
            onClick={() => setActiveClientTab('standards_lookup')}
            className={`flex items-center gap-1 px-2.5 py-1 rounded-md font-semibold ${
              activeClientTab === 'standards_lookup' ? 'bg-blue-600 text-white' : 'text-slate-400'
            }`}
          >
